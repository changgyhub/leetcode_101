...
在一個未排序的數組中，找到第 $k$ 大的數字。

### 輸入輸出範例

輸入一個數組和一個目標值 $k$，輸出第 $k$ 大的數字。題目保證一定有解。
...
`快速選擇` 通常用於解決 k-th Element 問題，可以在平均 $O(n)$ 時間複雜度和 $O(1)$ 空間複雜度下完成求解。快速選擇的實現與快速排序相似，但只需要找到第 $k$ 大的中樞（pivot），不需要對中樞左右再進行排序。與快速排序一樣，快速選擇一般需要先將數組打亂，否則最壞情況下的時間複雜度為 $O(n^2)$。
