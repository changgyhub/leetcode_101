...
`雜湊表（hash table, hash map）`，又稱散列表，使用 $O(n)$ 空間複雜度存儲資料，通過雜湊函數（hash function）映射位置，從而實現近似 O(1) 時間複雜度的插入、查詢、刪除等操作。雜湊表可以用來統計頻率、記錄內容等等。

如果元素是有限的，且範圍不大，那麼可以用一個固定大小的陣列來存儲或統計元素。例如，我們需要統計一個字串中所有字母的出現次數，則可以用一個長度為 26 的陣列來進行統計，其雜湊函數即為字母在字母表中的位置，這樣空間複雜度就可以降低為常數。
