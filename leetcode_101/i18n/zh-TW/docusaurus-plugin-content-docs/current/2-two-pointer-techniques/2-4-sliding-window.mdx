...
本題使用滑動窗口求解，即兩個指針 `l` 和 `r` 都是從最左端向最右端移動，且 `l` 的位置一定在 `r` 的左邊或重合。在 C++ 解法中，使用了兩個長度為 128 的數組，`valid` 和 `freq`，用來映射字符（ASCII 僅包含 128 個字符）。其中 `valid` 表示每個字符在 `t` 中是否存在，而 `freq` 表示目前 `t` 中每個字符在 `s` 的滑動窗口中缺少的數量：如果為正，則說明還缺少；如果為負，則說明有盈餘。在 Python 解法中，直接使用了 `Counter` 數據結構，同時統計 `t` 中存在的字符和其缺少的數量（也可以用 `dict` 替代）。
